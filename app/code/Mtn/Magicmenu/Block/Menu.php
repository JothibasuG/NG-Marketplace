...
class Menu extends \Magento\Catalog\Block\Navigation
{
     ...
     *
     * @var \Mtn\Magicmenu\Model\ResourceModel\Magicmenu\CollectionFactory
     */
    ...
    public function __construct(
        ...
        // +++++++++add new +++++++++
        \Mtn\Magicmenu\Helper\Data $helper,
        // \Mtn\Magicmenu\Model\CategoryFactory $categoryFactory,
    ...
    public function drawMainMenu()
    {
        ...
        foreach ($categories as $catTop){
            ...
            $desktopHtml[$idTop] = '<li class="level0 category-item level-top newww cat ' . $classTop . '"' . $options .'>' . $urlTop . $menu['desktop'] . '</li>';
    ...
    public function  getTreeCategories($categories, $itemPositionClassPrefix, $count='') // include Magic_Label and Maximal Depth
    {
        ...
        foreach($categories as $category) {
            ...
            $cclass ='';
            if($level == 3)$cclass=' hasChild parent ';
            $html .= '<li class="level' . ($level -2) . $childClass . ''.$cclass.'"><a href="' . $category->getUrl() . '"><span>' . $category->getName() . $count . $this->getCatLabel($category) . "</span></a>\n";
            $html .= $childHtml;
            $html .= $this->getStaticBlock($category->getEntityId().'right_proportion');;
            $html .= '</li>';
            $counter++;
        }
    ...
    {
        $url = '';
        $image = $this->_dirMedia . 'mtn/magicmenu/images/' . $category->getEntityId() .'.png';
        if(file_exists($image)) $url = $this->_urlMedia . 'mtn/magicmenu/images/' . $category->getEntityId() . '.png';
    ...
    {
        $url = '';
        $image = $this->_dirMedia . 'mtn/magicmenu/thumbnail/' . $category->getEntityId() .'.png';
        if(file_exists($image)) $url = $this->_urlMedia . 'mtn/magicmenu/thumbnail/' . $category->getEntityId() . '.png';
